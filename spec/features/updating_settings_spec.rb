...
describe "testing updating settings:" do
  before do
    ...
    visit settings_path
    fill_in 'Accepted threshold', with: 5
    fill_in 'Waitlist threshold', with: 5
  ...
  it "moves to settings view, fills in the form fields, clicks save, checks if settings got updated" do
    expect(Setting.get.accepted_threshold).to eq(5)
    expect(Setting.get.waitlist_threshold).to eq(5)
